             ...
             size_t buf_len)      /* length of buf */
{
  ...
  else
  {
    switch(g_line)
    {
      ...
      case 12:
#ifndef FIXED_PRECISION
        snprintf(buf, buf_len, "G0 Z%.*f%s", _precision, _Z_up,"");  // modified
        //snprintf(buf, buf_len, "G0 Z%.*f%s", _precision, _Z_up, _verbose? " ( Pen-Up at start)":"");
#else
        snprintf(buf, buf_len, "G0 Z%.2f%s", _Z_up, _verbose? "");            // modified
    ...
    */
    if (c)
    {
      ...
      if(!current_glyph) /*load new glyph */
      {
        ...
        if (_verbose)                                                                            // modified
          snprintf(buf, buf_len, ";");                                                           // original is in next line
        //snprintf(buf, buf_len, ";%c = %s",c, glyph_ptr); 
        //grbl hast problems with ;x comments and shows error "expected command letter"             modified
        //else
      ...
      else if(!(*glyph_ptr)) /*end of glyph*/
      {
        ...
        _Z = _Z_up;
#ifndef FIXED_PRECISION
        snprintf(buf, buf_len, "G0 Z%.*f%s", _precision, _Z,""); // modified
        //snprintf(buf, buf_len, "G0 Z%.*f%s", _precision, _Z, _verbose? "":""); 
#else
        snprintf(buf, buf_len, "G0 Z%.2f%s", _Z,""); // modified
        //snprintf(buf, buf_len, "G0 Z%.2f%s", _Z, _verbose? "":"");
#endif
        pen_state=Up;
        return g_line++;
      }
      ...
      else
      {
        ...
        else
        {
          ...
          else
          {
            ...
            _Z = _Z_down;
#ifndef FIXED_PRECISION
            snprintf(buf, buf_len, "G1 Z%.*f%s", _precision, _Z,""); // modified
            // snprintf(buf, buf_len, "G1 Z%.*f%s", _precision, _Z, _verbose? "":""); 
#else
            snprintf(buf, buf_len, "G1 Z%.2f%s", _Z,""); // modified
            //snprintf(buf, buf_len, "G1 Z%.2f%s", _Z, _verbose? "":"");
#endif
            pen_state = Down;
            pen_above_pos=0;
            return g_line++;
